...
# Assuming 'model' is in '/home/zidongjiashi/users/fyz/work/CoFiI2P/'
...
if __name__=='__main__':
    ...
    # parser.add_argument("ckpt", type=str, nargs="?", default="/home/zidongjiashi/users/fyz/work/CoFiI2P/ckpt/cofii2p_kitti.t7", help="checkpoint path")
    # parser.add_argument("dataset", type=str, nargs="?", default="kitti", help="eval dataset")
    parser.add_argument("ckpt", type=str, help="checkpoint path")
    parser.add_argument("dataset", type=str, help="eval dataset")
    parser.add_argument("--eval_path", type=str, default="./eval_past", help="path for evaluation files")
    parser.add_argument("--log_file", type=str, default="./eval_log_past", help="path for log file")
    ...
    with torch.no_grad():
        for step, data in enumerate(testloader):
            ...
            img_features, pc_features, coarse_img_score, coarse_pc_score, fine_img_feature_patch, fine_pc_inline_feature, fine_center_xy, coarse_pc_points = model(pc_data_dict, img, fine_center_kpt_coors, fine_xy, fine_pc_inline_index, mode)  # [1, 128, 20, 64] ,[128, 2560]
            ...
            if is_success is True:
                ...
                print(f'Step {step}: angles_diff={angles_diff}, t_diff={t_diff}')
                logging.info(f'Step {step}: angles_diff={angles_diff}, t_diff={t_diff}')
            ...
            save_dict['img'] = img  # [1, 3, 160, 512]
            print(eval_path / str('%06d.npy'%(step)))
            logging.info(f'Saving results to {eval_path / str("%06d.npy" % step)}')
        ...
        logging.info(f'Success num / total num: {success_num}/{len(testloader.dataset)}')
        logging.info(f'Mean angles_diff: {np.mean(angles_diff_set)}, Mean t_diff: {np.mean(t_diff_set)}')
