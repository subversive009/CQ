...
class CoFiI2P(nn.Module):
    def __init__(self,opt):
        ...
        # 修改 transformer 初始化，启用几何嵌入
        self.transformer = LocalFeatureTransformer(
            D_MODEL=128, 
            NHEAD=4, 
            LAYER_NAMES=['self', 'cross'] * 4, 
            ATTENTION='full',
            use_geo_embedding=True,  # 启用几何嵌入
            geo_config={
                'hidden_dim': 128,
                'sigma_d': 0.2,  # 距离量化参数
                'sigma_a': 15,   # 角度量化参数
    ...
    def forward(self,pc_data_dict,img, fine_center_kpt_coors,fine_xy, fine_pc_inline_index, mode):
        ...
        # 获取点云坐标
        pc_coords = pc_data_dict['points'][-1].unsqueeze(0)  # [1, N, 3]
        ...
        # 传递点云坐标到 transformer
        image_feature_mid, pc_features_fusion = self.transformer(image_feature_8, pc_feature_8, pc_coords)
